...
public class App {
    ...
    private static void guardarBibtex(List<Articulo> articulos, String outputPath, boolean bol) throws IOException {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputPath))) {
            for (int i = 0; i < articulos.size(); i++) {
                writer.write(articulos.get(i).aBibtex( i + 1, bol));
                writer.newLine();
                writer.newLine();
            }
        }
